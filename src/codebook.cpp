...
constexpr quint64 HEADER_SIZE = sizeof(CodeBook::Header);
// Threshold for key material warning (20%)
constexpr double LOW_KEY_THRESHOLD = 0.20;

CodeBook::CodeBook(QObject *parent)
...
bool CodeBook::open(const QString &filename)
{
    ...
    if (!file.open(QIODevice::ReadWrite)) {
        emit error(tr("Failed to open codebook: %1").arg(file.errorString()));
        return false;
    }
    
    // Check if file size is at least as large as the header
    if (file.size() < static_cast<qint64>(HEADER_SIZE)) {
        emit error(tr("Invalid codebook file format"));
        file.close();
        return false;
    }
    ...
    if (!validateIntegrity()) {
        emit error(tr("Codebook integrity check failed"));
        file.close();
        return false;
    }
...
bool CodeBook::create(const QString &filename, quint64 size)
{
    QMutexLocker locker(&accessMutex);
    ...
    if (size < 1024) {
        emit error(tr("Codebook size is too small"));
        return false;
    }
    ...
    if (!file.open(QIODevice::ReadWrite | QIODevice::Truncate)) {
        emit error(tr("Failed to create codebook: %1").arg(file.errorString()));
        return false;
    }
...
bool CodeBook::isOpen() const
{
    return file.isOpen();
}

QByteArray CodeBook::getKeyMaterial(quint64 position, quint64 length)
{
    QMutexLocker locker(&accessMutex);
    ...
    if (position + length > header.totalSize) {
        emit error(tr("Requested key material exceeds codebook size"));
        return QByteArray();
    }
    ...
    if (!file.seek(filePosition)) {
        emit error(tr("Failed to seek in codebook: %1").arg(file.errorString()));
        return QByteArray();
    }
...
bool CodeBook::markAsUsed(quint64 position, quint64 length)
{
    QMutexLocker locker(&accessMutex);
    ...
    // Check if position and length are valid
    if (position + length > header.totalSize) {
        emit error(tr("Marked key material exceeds codebook size"));
        return false;
    }
...
quint64 CodeBook::getCurrentPosition() const
{
    return header.currentPosition;
}

quint64 CodeBook::getTotalSize() const
{
    return header.totalSize;
}

quint64 CodeBook::getUnusedSize() const
{
    return header.totalSize - header.currentPosition;
}

double CodeBook::getPercentageRemaining() const
{
...
QByteArray CodeBook::generateRandomKeyMaterial(quint64 size)
{
...
bool CodeBook::createCompartment(const QString &name, quint64 size)
{
    QMutexLocker locker(&accessMutex);
...
bool CodeBook::lockCompartment(const QString &name)
{
    QMutexLocker locker(&accessMutex);
...
bool CodeBook::unlockCompartment(const QString &name, const QByteArray &key)
{
    QMutexLocker locker(&accessMutex);
...
QList<QString> CodeBook::getCompartmentNames() const
{
    QList<QString> names;
...
QByteArray CodeBook::getAuthenticationMaterial(quint64 length)
{
    QMutexLocker locker(&accessMutex);
...
bool CodeBook::markAuthenticationUsed(quint64 position, quint64 length)
{
...
void CodeBook::setEmergencyDestruction(const QString &code)
{
    QMutexLocker locker(&accessMutex);
...
bool CodeBook::executeEmergencyProtocol(const QString &code)
{
    QMutexLocker locker(&accessMutex);
...
void CodeBook::setDuressCode(const QString &code)
{
    QMutexLocker locker(&accessMutex);
...
bool CodeBook::isDuressCode(const QString &code) const
{
...
bool CodeBook::readHeader()
{
    ...
    if (file.read(reinterpret_cast<char*>(&header), sizeof(Header)) != sizeof(Header)) {
        emit error(tr("Failed to read codebook header: %1").arg(file.errorString()));
        return false;
    }
    
    // Check format version
    if (header.formatVersion > CURRENT_FORMAT_VERSION) {
        emit error(tr("Unsupported codebook format version %1").arg(header.formatVersion));
        return false;
    }
    ...
    if (calculatedChecksum != expectedChecksum) {
        emit error(tr("Codebook header checksum mismatch"));
        return false;
    }
...
bool CodeBook::writeHeader()
{
    ...
    if (file.write(reinterpret_cast<char*>(&header), sizeof(Header)) != sizeof(Header)) {
        emit error(tr("Failed to write codebook header: %1").arg(file.errorString()));
        return false;
    }
...
bool CodeBook::readCompartments()
{
...
bool CodeBook::writeCompartments()
{
    // Check if there are any compartments
...
quint32 CodeBook::calculateHeaderChecksum() const
{
...
bool CodeBook::validateIntegrity()
{
    ...
    if (file.size() != static_cast<qint64>(expectedFileSize)) {
        emit error(tr("Codebook file size mismatch"));
        return false;
    }
...
bool CodeBook::secureWipe(quint64 offset, quint64 length)
{
...
QByteArray CodeBook::hashCode(const QString &code)
{
